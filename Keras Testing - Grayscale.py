...
for n, id_ in tqdm(enumerate(train_ids), total=len(train_ids)):
    ...
    if (0, 0, 0) in test or len(test) > 5:
        print(id_)
        print(test)
        imshow(mask)
...
c1 = Conv2D(256, (21,21), activation='elu', kernel_initializer='he_normal', padding='same') (s)
d1 = Dropout(0.2) (c1)
c2 = Conv2D(512, (1,1), activation='elu', kernel_initializer='he_normal', padding='same') (d1)
d2 = Dropout(0.2) (c2)
c3 = Conv2D(128, (1,1), activation='elu', kernel_initializer='he_normal', padding='same') (d2)
outputs = Conv2D(3, (1, 1), activation='softmax', kernel_initializer='he_normal', padding='same') (c3)
...
checkpointer = ModelCheckpoint('model-dsbowl2018-gray-v1.2-0.h5', verbose=1, save_best_only=True)
results = model.fit(X_train_gray, Y_train, validation_split=0.1, batch_size=16, epochs=50,
